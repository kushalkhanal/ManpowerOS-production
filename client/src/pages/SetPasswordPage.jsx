...
const SetPasswordPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!PASSWORD_RULES.test(password)) {
      setFieldError('Password must be 8+ chars with uppercase, lowercase, number, and special character (@$!%*?&)');
      return;
    }
    if (password !== confirm) {
    ...
    setSubmitting(true);
    try {
      const { data } = await api.post('/auth/complete-invite', { token, newPassword: password });
      setSession({ token: data.token, user: data.user, agency: data.agency });
      setState('done');
      showToast.success('Password set — welcome to ManpowerOS!');
      setTimeout(() => navigate('/dashboard'), 1500);
    } catch (err) {
      setFieldError(err.response?.data?.message || 'Failed to set password. The link may have expired.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center">
          ...
          <p className="text-gray-500 text-sm mb-6">
            This invite link has expired or is no longer valid. Please ask your administrator to send a new invite.
          </p>
        ...
        <div className="text-center mb-6">
          ...
          {inviteeInfo && (
            <p className="text-gray-500 text-sm mt-1">
              Welcome, <span className="font-medium text-gray-700">{inviteeInfo.name}</span>!
              Setting password for <span className="font-medium text-gray-700">{inviteeInfo.email}</span>
            </p>
          )}
        </div>
