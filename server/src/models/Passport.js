...
const passportSchema = new mongoose.Schema({
  ...
  custodyStatus: {
    type: String,
    enum: ['with_agency', 'returned_to_candidate', 'submitted_embassy', 'lost'],
    default: 'with_agency'
  },
  location: String,
  notes: String,
...
passportSchema.virtual('validityMonths').get(function() {
  ...
  const now = new Date();
  const months = (this.expiryDate.getFullYear() - now.getFullYear()) * 12 + (this.expiryDate.getMonth() - now.getMonth());
