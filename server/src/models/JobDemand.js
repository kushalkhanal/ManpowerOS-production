...
jobDemandSchema.virtual("demandLetterExpiryDateBS").get(function () {
  return this.demandLetterExpiryDate
    ? formatBSDisplay(this.demandLetterExpiryDate)
    : null;
});

jobDemandSchema.virtual("purbaSwukritiDateBS").get(function () {
  return this.purbaSwukritiDate
    ? formatBSDisplay(this.purbaSwukritiDate)
    : null;
});

jobDemandSchema.virtual("purbaSwukritiExpiryDateBS").get(function () {
  return this.purbaSwukritiExpiryDate
    ? formatBSDisplay(this.purbaSwukritiExpiryDate)
    : null;
});

jobDemandSchema.pre("save", function (next) {
  if (
    this.demandLetterExpiryDate &&
    new Date(this.demandLetterExpiryDate) < new Date() &&
    this.status === "active"
  ) {
    this.status = "expired";
  }
