...
const medicalBaseSchema = z.object({
  ...
  scheduledDate: z
    ...
    .refine(
      (date) =>
        date >=
        new Date(new Date().setHours(0, 0, 0, 0) - 30 * 24 * 60 * 60 * 1000),
      {
...
export const medicalSchema = medicalBaseSchema
  .refine(
    (data) => {
  ...
  .refine(
    (data) => {
      ...
      if (data.conductedDate && data.reportExpiryDate) {
        const daysDiff =
          (data.reportExpiryDate - data.conductedDate) / (24 * 60 * 60 * 1000);
    ...
    {
      message:
        "Report validity should be between 1 month and 1 year from conducted date",
      path: ["reportExpiryDate"],
    },
  );
...
// Schema with refinements for creation
export const orientationSchema = orientationBaseSchema
  .refine(
    (data) => {
