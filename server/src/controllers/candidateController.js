...
const DOCUMENT_CHECKLIST_KEYS = {
  passport_collection: ['passport_received', 'passport_verified', 'passport_renewed'],
  medical: ['medical_scheduled', 'medical_conducted', 'result_received', 'report_uploaded'],
  insurance: ['insurance_paid', 'policy_generated', 'ssf_registered', 'ssf_receipt'],
  calling_visa: ['demand_letter_confirmed', 'visa_number_obtained', 'visa_approval_confirmed'],
  visa: ['embassy_submitted', 'visa_stamped', 'passport_returned'],
  fee: ['fee_agreed', 'partial_payment', 'full_payment', 'receipt_issued'],
  flight: ['ticket_booked', 'ticket_confirmed', 'airline_confirmed', 'airport_time_set'],
  dofe: ['orientation_certified', 'insurance_verified', 'medical_cert_valid', 'ssf_confirmed', 'shram_received', 'e_sticker_received'],
  doc_prep: ['all_docs_compiled', 'ticket_ready', 'briefing_done', 'docs_handed'],
...
const enrichWithCompliance = async (candidates) => {
  ...
  const [medicals, orientations, insuranceSsfs] = await Promise.all([
    ...
    Orientation.find({ candidateId: { $in: ids } }, 'candidateId completionStatus').lean(),
    InsuranceSsf.find({ candidateId: { $in: ids } }, 'candidateId insurancePaidDate insurancePolicyNumber ssfPaidDate ssfRegistrationNumber welfareFundPaid').lean(),
  ]);
  const medMap = {}, oriMap = {}, insMap = {};
...
const createCandidate = asyncHandler(async (req, res) => {
  const {
    fullName, fullNameNepali, dateOfBirth, gender, nationalIdNumber,
    phone, alternatePhone, email,
    permanentProvince, permanentDistrict, permanentMunicipality, permanentWardNo, temporaryAddress,
    education, skills, languagesKnown, workExperienceYears, previousCountry,
    desiredCountry, desiredJobCategory, referredBy, agentId,
    serviceFeeAgreed, serviceFeeReceived
  } = req.body;
  ...
  if (existingNid) {
    return res.status(400).json({ message: 'Candidate with this NID already exists' });
  }
  ...
  if (existingPhone) {
    return res.status(400).json({ message: 'A candidate with this phone number already exists' });
  }
  ...
  if (req.body.passportNumber) {
    ...
    if (existingPassport) {
      return res.status(400).json({ message: 'A candidate with this passport number already exists' });
    }
  }

  if (agentId) {
    const agent = await User.findById(agentId);
    if (!agent || agent.role !== 'agent' || agent.agencyId.toString() !== req.user.agencyId.toString()) {
      return res.status(400).json({ message: 'Invalid agent selected' });
    }
  }

  const paymentStatus = serviceFeeReceived >= serviceFeeAgreed ? 'paid' : 
                        serviceFeeReceived > 0 ? 'partial' : 'unpaid';

  const candidate = await Candidate.create(scopeData(req, {
    fullName, fullNameNepali, dateOfBirth, gender, nationalIdNumber,
    phone, alternatePhone, email,
    permanentProvince, permanentDistrict, permanentMunicipality, permanentWardNo, temporaryAddress,
    education, skills, languagesKnown, workExperienceYears, previousCountry,
  ...
  const populated = await Candidate.findById(candidate._id).populate('agentId', 'name');

  res.status(201).json(populated);
});

const getCandidateById = asyncHandler(async (req, res) => {
  const candidate = await Candidate.findOne(scopeFilter(req, {
    _id: req.params.id
  })).populate('agentId', 'name');
...
const updateCandidate = asyncHandler(async (req, res) => {
  ...
  if (updates.nationalIdNumber) {
    ...
    if (existing) {
      return res.status(400).json({ message: 'NID already exists for another candidate' });
    }
  }
  ...
  if (updates.phone) {
    ...
    if (existing) {
      return res.status(400).json({ message: 'Phone number already exists for another candidate' });
    }
  }
  ...
  if (updates.passportNumber) {
    ...
    if (existing) {
      return res.status(400).json({ message: 'Passport number already exists for another candidate' });
    }
  }
  ...
  if (updates.serviceFeeReceived !== undefined && updates.serviceFeeAgreed !== undefined) {
    updates.paymentStatus = updates.serviceFeeReceived >= updates.serviceFeeAgreed ? 'paid' : 
                             updates.serviceFeeReceived > 0 ? 'partial' : 'unpaid';
  }
  ...
  if (uploadedFiles.visaFile?.[0]) {
    if (oldCandidate?.visaFileUrl) orphanedFileUrls.push(oldCandidate.visaFileUrl);
    updates.visaFileUrl = uploadedFiles.visaFile[0].path;
  }
  if (uploadedFiles.feimsFile?.[0]) {
    if (oldCandidate?.feimsFileUrl) orphanedFileUrls.push(oldCandidate.feimsFileUrl);
    updates.feimsFileUrl = uploadedFiles.feimsFile[0].path;
  }
  if (uploadedFiles.departureFile?.[0]) {
    if (oldCandidate?.departureFileUrl) orphanedFileUrls.push(oldCandidate.departureFileUrl);
  ...
  const updatedCandidate = await Candidate.findById(candidate._id).populate('agentId', 'name');
  ...
  if (updates.visaNumber || updates.visaFileUrl) {
    await logActivity({
      ...
      details: updates.visaNumber ? `Visa#: ${updates.visaNumber}` : 'Visa details updated',
      fileUrl: updates.visaFileUrl
    });
  }

  if (updates.shramSwikritiNumber || updates.eStickerNumber || updates.feimsFileUrl) {
    await logActivity({
      candidateId: candidate._id,
      agencyId: req.user.agencyId,
      userId: req.user.userId,
      ...
      details: updates.shramSwikritiNumber ? `Shram Swikriti#: ${updates.shramSwikritiNumber}` : 'FEIMS details updated',
  ...
  if (updates.flightDate || updates.flightNumber || updates.departureFileUrl) {
    await logActivity({
      ...
      details: updates.flightNumber ? `Flight: ${updates.flightNumber}` : 'Departure details updated',
...
const deleteCandidate = asyncHandler(async (req, res) => {
  if (req.user.role !== 'admin' && req.user.role !== 'superadmin') {
    return res.status(403).json({ message: 'Insufficient permissions to delete candidates' });
  }
  ...
  const candidateFileFields = ['visaFileUrl', 'feimsFileUrl', 'vlnFileUrl', 'plksFileUrl', 'departureFileUrl'];
  ...
  const [medicalDocs, orientationDocs, insuranceDocs] = await Promise.all([
    ...
    Orientation.find({ candidateId: candidate._id }).select('certificateFileUrl attendanceSheetUrl').lean(),
    InsuranceSsf.find({ candidateId: candidate._id }).select('insurancePaidReceiptUrl ssfReceiptUrl').lean()
  ]);
  medicalDocs.forEach((d) => d.reportFileUrl && filesToDelete.push(d.reportFileUrl));
  ...
  insuranceDocs.forEach((d) => {
    if (d.insurancePaidReceiptUrl) filesToDelete.push(d.insurancePaidReceiptUrl);
  ...
  if (filesToDelete.length > 0) {
    Promise.all(
      filesToDelete
        ...
        .filter(Boolean)
        .map((publicId) => deleteCloudinaryFile(publicId))
    ).catch((err) => logger.error('Candidate delete: Cloudinary cleanup failed', err));
  }

  res.status(200).json({ message: 'Candidate and all related data deleted successfully' });
});
...
const exportCandidates = asyncHandler(async (req, res) => {
  ...
  const headers = [
    'Full Name', 'Phone', 'NID', 'Status', 'Country',
    'Job Category', 'Agent', 'Service Fee', 'Paid', 'Registered Date'
  ];
...
const getCandidateKanban = asyncHandler(async (req, res) => {
  ...
  const [medical, orientation, insuranceSsf, feeTransactions] = await Promise.all([
    ...
    FeeTransaction.find({ candidateId: id, transactionType: 'service_fee', direction: 'received' })
      .sort({ paidAt: -1 })
      .limit(20)
  ]);
  ...
  if (candidate.passportId) {
    ...
    passport = await Passport.findOne({ passportNumber: candidate.passportNumber, agencyId }).lean();
  }
  ...
  const passportCollectionColumn = buildPassportCollectionColumn(passport, checklistMap, stageNotes);
  const medicalColumn = buildMedicalColumn(medical, checklistMap, stageNotes);
  const insuranceColumn = buildInsuranceColumn(insuranceSsf, checklistMap, stageNotes);
  const callingVisaColumn = buildCallingVisaColumn(candidate, demand, checklistMap, stageNotes);
  const visaColumn = buildVisaColumn(candidate, passport, checklistMap, stageNotes);
  const feeColumn = buildFeeColumn(feeTransactions, candidate.serviceFeeAgreed, checklistMap, stageNotes);
  const flightColumn = buildFlightColumn(candidate, checklistMap, stageNotes);
  const dofeColumn = buildDofeColumn(candidate, medical, orientation, insuranceSsf, checklistMap, stageNotes);
  const docPrepColumn = buildDocPrepColumn(candidate, checklistMap, stageNotes);
  const departureColumn = buildDepartureColumn(candidate, checklistMap, stageNotes);
  ...
  res.status(200).json({
    candidate: {
      ...
      demandJobCategory: demand?.jobCategory || candidate.desiredJobCategory || ''
    },
    passport,
    demand,
    columns,
...
const buildPassportCollectionColumn = (passport, checklistMap = {}, stageNotes = {}) => {
  const withAgency = passport?.custodyStatus === 'with_agency';
  const checkItems = [
    { key: 'passport_received', label: 'Passport received from candidate', ...getChecklistMeta(checklistMap, 'passport_collection', 'passport_received', withAgency) },
    { key: 'passport_verified', label: 'Passport details verified', ...getChecklistMeta(checklistMap, 'passport_collection', 'passport_verified', withAgency && !!passport?.passportNumber) },
    { key: 'passport_renewed', label: 'Renewal done (if needed)', ...getChecklistMeta(checklistMap, 'passport_collection', 'passport_renewed', false) }
  ];

  let status = 'pending';
  if (passport) {
    if (passport.custodyStatus === 'submitted_embassy') status = 'in_progress';
    else if (passport.custodyStatus === 'with_agency') status = checkItems.every(i => i.done) ? 'complete' : 'in_progress';
    else status = 'in_progress';
  }

  let daysUntilExpiry = null;
  if (passport?.expiryDate) {
    daysUntilExpiry = Math.ceil((new Date(passport.expiryDate) - new Date()) / (1000 * 60 * 60 * 24));
  ...
  return {
    ...
    icon: 'passport',
    phase: 1, phaseLabel: 'Phase 1 — Pre-processing',
    status,
    requiredFor: 'Medical check',
    data: passport,
    uploads: passport?.scannedImageUrl ? [{ label: 'Passport Scan', url: passport.scannedImageUrl }] : [],
...
const buildMedicalColumn = (medical, checklistMap = {}, stageNotes = {}) => {
  const checkItems = [
    { key: 'medical_scheduled', label: 'Medical scheduled', ...getChecklistMeta(checklistMap, 'medical', 'medical_scheduled', !!medical?.scheduledDate) },
    { key: 'medical_conducted', label: 'Medical conducted', ...getChecklistMeta(checklistMap, 'medical', 'medical_conducted', !!medical?.conductedDate) },
    { key: 'result_received', label: 'Result received', ...getChecklistMeta(checklistMap, 'medical', 'result_received', !!medical?.result && medical.result !== 'pending') },
    { key: 'report_uploaded', label: 'Report uploaded', ...getChecklistMeta(checklistMap, 'medical', 'report_uploaded', !!medical?.reportFileUrl) }
  ];

  let status = 'pending';
  if (medical) {
    if (medical.result === 'unfit') status = 'blocked';
    else if (medical.result === 'fit' && medical.reportFileUrl) status = 'complete';
    else if (medical.scheduledDate || medical.conductedDate) status = 'in_progress';
  }

  let daysUntilExpiry = null;
  if (medical?.reportExpiryDate) {
    daysUntilExpiry = Math.ceil((new Date(medical.reportExpiryDate) - new Date()) / (1000 * 60 * 60 * 24));
    if (daysUntilExpiry < 0) status = 'expired';
  ...
  return {
    ...
    data: medical,
    uploads: medical?.reportFileUrl ? [{ label: 'Medical Report', url: medical.reportFileUrl }] : [],
    hasOverrides: checkItems.some((i) => i.overridden),
...
const buildInsuranceColumn = (insuranceSsf, checklistMap = {}, stageNotes = {}) => {
  const checkItems = [
    { key: 'insurance_paid', label: 'Insurance paid', ...getChecklistMeta(checklistMap, 'insurance', 'insurance_paid', !!insuranceSsf?.insurancePaidDate) },
    { key: 'policy_generated', label: 'Policy generated', ...getChecklistMeta(checklistMap, 'insurance', 'policy_generated', !!insuranceSsf?.insurancePolicyNumber) },
    { key: 'ssf_registered', label: 'SSF registered', ...getChecklistMeta(checklistMap, 'insurance', 'ssf_registered', !!insuranceSsf?.ssfPaidDate) },
    { key: 'ssf_receipt', label: 'SSF receipt', ...getChecklistMeta(checklistMap, 'insurance', 'ssf_receipt', !!insuranceSsf?.ssfReceiptNumber) }
  ];

  let status = 'pending';
  if (insuranceSsf) {
    if (insuranceSsf.overallStatus === 'completed') status = 'complete';
    else if (insuranceSsf.overallStatus === 'partially_done') status = 'in_progress';
  }

  let daysUntilExpiry = null;
  if (insuranceSsf?.insuranceExpiryDate) {
    daysUntilExpiry = Math.ceil((new Date(insuranceSsf.insuranceExpiryDate) - new Date()) / (1000 * 60 * 60 * 24));
    if (daysUntilExpiry < 0) status = 'expired';
    else if (daysUntilExpiry < 30 && status === 'complete') status = 'expiring';
  }
  ...
  return {
    ...
    icon: 'shield',
    phase: 2, phaseLabel: 'Phase 2 — Insurance & Visa',
    status,
    requiredFor: 'Calling visa',
    data: insuranceSsf,
    uploads: [
      ...(insuranceSsf?.insurancePaidReceiptUrl ? [{ label: 'Insurance Receipt', url: insuranceSsf.insurancePaidReceiptUrl }] : []),
      ...(insuranceSsf?.ssfReceiptUrl ? [{ label: 'SSF Receipt', url: insuranceSsf.ssfReceiptUrl }] : [])
    ],
    hasOverrides: checkItems.some((i) => i.overridden),
...
const buildCallingVisaColumn = (candidate, demand, checklistMap = {}, stageNotes = {}) => {
  const checkItems = [
    { key: 'demand_letter_confirmed', label: 'Demand letter confirmed', ...getChecklistMeta(checklistMap, 'calling_visa', 'demand_letter_confirmed', !!candidate.demandId) },
    { key: 'visa_number_obtained', label: 'Visa number obtained from employer', ...getChecklistMeta(checklistMap, 'calling_visa', 'visa_number_obtained', !!candidate.visaNumber) },
    { key: 'visa_approval_confirmed', label: 'Visa approval confirmed', ...getChecklistMeta(checklistMap, 'calling_visa', 'visa_approval_confirmed', false) }
  ];

  let status = 'pending';
  if (candidate.demandId) {
    if (candidate.visaNumber) status = checkItems.every(i => i.done) ? 'complete' : 'in_progress';
    else status = 'in_progress';
  }
  ...
  return {
    id: 'calling_visa',
    title: 'Calling Visa',
    subtitle: demand?.employerCountry ? `Demand: ${demand.employerCountry}` : 'Employer demand letter',
    icon: 'file',
    phase: 2, phaseLabel: 'Phase 2 — Insurance & Visa',
    status,
...
const buildVisaColumn = (candidate, passport, checklistMap = {}, stageNotes = {}) => {
  const checkItems = [
    { key: 'embassy_submitted', label: 'Passport submitted to embassy', ...getChecklistMeta(checklistMap, 'visa', 'embassy_submitted', passport?.custodyStatus === 'submitted_embassy') },
    { key: 'visa_stamped', label: 'Visa stamped on passport', ...getChecklistMeta(checklistMap, 'visa', 'visa_stamped', !!candidate.visaFileUrl) },
    { key: 'passport_returned', label: 'Passport returned to office', ...getChecklistMeta(checklistMap, 'visa', 'passport_returned', !!candidate.visaFileUrl && passport?.custodyStatus === 'with_agency') }
  ];

  let status = 'pending';
  if (candidate.visaNumber) {
    if (candidate.visaFileUrl) status = 'complete';
    else if (passport?.custodyStatus === 'submitted_embassy') status = 'in_progress';
    else status = 'in_progress';
  }
  ...
  return {
    ...
    uploads: candidate.visaFileUrl ? [{ label: 'Visa Document', url: candidate.visaFileUrl }] : [],
    hasOverrides: checkItems.some(i => i.overridden),
    lastUpdatedAt: candidate.updatedAt || null,
    checkItems,
    completedCount: checkItems.filter(i => i.done).length,
    ...
    daysUntilExpiry: candidate.visaExpiryDate ? Math.ceil((new Date(candidate.visaExpiryDate) - new Date()) / (1000 * 60 * 60 * 24)) : null,
...
// ─── Phase 2: Service Fee ───────────────────────────────────────────────────
const buildFeeColumn = (transactions, serviceFeeAgreed, checklistMap = {}, stageNotes = {}) => {
  const totalReceived = transactions.reduce((sum, t) => sum + t.amountNPR, 0);
  const checkItems = [
    { key: 'fee_agreed', label: 'Fee agreed', ...getChecklistMeta(checklistMap, 'fee', 'fee_agreed', !!serviceFeeAgreed) },
    { key: 'partial_payment', label: 'Partial payment', ...getChecklistMeta(checklistMap, 'fee', 'partial_payment', totalReceived > 0) },
    { key: 'full_payment', label: 'Full payment', ...getChecklistMeta(checklistMap, 'fee', 'full_payment', totalReceived >= (serviceFeeAgreed || 0)) },
    { key: 'receipt_issued', label: 'Receipt issued', ...getChecklistMeta(checklistMap, 'fee', 'receipt_issued', transactions.some(t => t.receiptUrl)) }
  ];
...
const buildFlightColumn = (candidate, checklistMap = {}, stageNotes = {}) => {
  const checkItems = [
    { key: 'ticket_booked', label: 'Flight ticket booked', ...getChecklistMeta(checklistMap, 'flight', 'ticket_booked', !!candidate.flightDate) },
    { key: 'ticket_confirmed', label: 'Flight number confirmed', ...getChecklistMeta(checklistMap, 'flight', 'ticket_confirmed', !!candidate.flightNumber) },
    { key: 'airline_confirmed', label: 'Airline confirmed', ...getChecklistMeta(checklistMap, 'flight', 'airline_confirmed', !!candidate.airline) },
    { key: 'airport_time_set', label: 'Airport report time set', ...getChecklistMeta(checklistMap, 'flight', 'airport_time_set', !!candidate.airportReportingTime) }
  ];

  let status = 'pending';
  if (candidate.flightDate) {
    status = (candidate.flightNumber && candidate.airline) ? 'complete' : 'in_progress';
  }
  ...
  return {
    ...
    subtitle: candidate.flightDate
      ? (`${candidate.airline || ''} ${candidate.flightNumber || ''}`.trim() || new Date(candidate.flightDate).toLocaleDateString('en-GB'))
      : 'Book departure flight',
    icon: 'plane',
    phase: 3, phaseLabel: 'Phase 3 — Travel Booking',
    status,
    ...
    uploads: candidate.departureFileUrl ? [{ label: 'Flight Ticket', url: candidate.departureFileUrl }] : [],
    hasOverrides: checkItems.some(i => i.overridden),
    lastUpdatedAt: candidate.updatedAt || null,
    checkItems,
    completedCount: checkItems.filter(i => i.done).length,
    ...
    daysUntilExpiry: candidate.flightDate ? Math.ceil((new Date(candidate.flightDate) - new Date()) / (1000 * 60 * 60 * 24)) : null,
...
const buildDofeColumn = (candidate, medical, orientation, insuranceSsf, checklistMap = {}, stageNotes = {}) => {
  const medicalValid = medical?.result === 'fit' && (!medical?.reportExpiryDate || new Date(medical.reportExpiryDate) > new Date());

  const checkItems = [
    { key: 'orientation_certified', label: 'Orientation certified (PDOT)', ...getChecklistMeta(checklistMap, 'dofe', 'orientation_certified', orientation?.completionStatus === 'completed' && !!orientation?.certificateFileUrl) },
    { key: 'insurance_verified', label: 'Insurance verified at DoFE', ...getChecklistMeta(checklistMap, 'dofe', 'insurance_verified', insuranceSsf?.overallStatus === 'completed') },
    { key: 'medical_cert_valid', label: 'Medical certificate valid', ...getChecklistMeta(checklistMap, 'dofe', 'medical_cert_valid', medicalValid) },
    { key: 'ssf_confirmed', label: 'SSF (Social Security Fund)', ...getChecklistMeta(checklistMap, 'dofe', 'ssf_confirmed', !!insuranceSsf?.ssfReceiptNumber) },
    { key: 'shram_received', label: 'Shram Swikriti received', ...getChecklistMeta(checklistMap, 'dofe', 'shram_received', !!candidate.shramSwikritiNumber) },
    { key: 'e_sticker_received', label: 'E-Sticker received', ...getChecklistMeta(checklistMap, 'dofe', 'e_sticker_received', !!candidate.eStickerNumber) }
  ];

  let status = 'pending';
  if (candidate.shramSwikritiNumber && candidate.eStickerNumber) {
    status = 'complete';
  } else if (orientation || candidate.shramSwikritiNumber || candidate.eStickerNumber) {
    status = 'in_progress';
  }
  ...
  return {
    ...
    subtitle: candidate.shramSwikritiNumber ? `Shram: ${candidate.shramSwikritiNumber}` : 'Shram Swikriti',
    icon: 'shield-check',
    phase: 4, phaseLabel: 'Phase 4 — DoFE / FEIMS Clearance',
    status,
    ...
    uploads: [
      ...(candidate.feimsFileUrl ? [{ label: 'FEIMS Document', url: candidate.feimsFileUrl }] : []),
      ...(orientation?.certificateFileUrl ? [{ label: 'Orientation Certificate', url: orientation.certificateFileUrl }] : [])
    ],
    hasOverrides: checkItems.some(i => i.overridden),
    lastUpdatedAt: candidate.updatedAt || null,
    checkItems,
...
const buildDocPrepColumn = (candidate, checklistMap = {}, stageNotes = {}) => {
  const checkItems = [
    { key: 'all_docs_compiled', label: 'All documents compiled', ...getChecklistMeta(checklistMap, 'doc_prep', 'all_docs_compiled', false) },
    { key: 'ticket_ready', label: 'Flight ticket printed / ready', ...getChecklistMeta(checklistMap, 'doc_prep', 'ticket_ready', false) },
    { key: 'briefing_done', label: 'Pre-departure briefing done', ...getChecklistMeta(checklistMap, 'doc_prep', 'briefing_done', false) },
    { key: 'docs_handed', label: 'Docs handed to candidate', ...getChecklistMeta(checklistMap, 'doc_prep', 'docs_handed', false) }
  ];
  ...
  return {
    ...
    icon: 'clipboard',
    phase: 5, phaseLabel: 'Phase 5 — Final Departure',
    status,
    requiredFor: 'Final departure',
    data: null,
    uploads: [],
...
const buildDepartureColumn = (candidate, checklistMap = {}, stageNotes = {}) => {
  const checkItems = [
    { key: 'airport_reported', label: 'Reported at airport', ...getChecklistMeta(checklistMap, 'departure', 'airport_reported', false) },
    { key: 'flight_departed', label: 'Flight departed', ...getChecklistMeta(checklistMap, 'departure', 'flight_departed', !!candidate.departedAt) },
    { key: 'confirmation_received', label: 'Safe arrival confirmed', ...getChecklistMeta(checklistMap, 'departure', 'confirmation_received', false) }
  ];
  ...
  return {
    ...
    subtitle: candidate.flightDate ? new Date(candidate.flightDate).toLocaleDateString('en-GB') : 'Departure date',
    icon: 'plane-takeoff',
    phase: 5, phaseLabel: 'Phase 5 — Final Departure',
    status,
...
const markColumnComplete = asyncHandler(async (req, res) => {
  ...
  const validColumns = ['passport_collection', 'medical', 'insurance', 'calling_visa', 'visa', 'fee', 'flight', 'dofe', 'doc_prep', 'departure'];
...
const toggleChecklistItem = asyncHandler(async (req, res) => {
  ...
  const normalizedDone = typeof done === 'boolean' ? done : !candidate.documentChecklist?.get(`${columnId}__${itemKey}`);
  const mapKey = `${columnId}__${itemKey}`;
...
export default {
  ...
  unassignFromDemand: asyncHandler(async (req, res) => {
    ...
    if (!demandId) {
      return res.status(400).json({ message: 'Candidate is not assigned to any demand' });
    }
    ...
    res.status(200).json({ message: 'Candidate unassigned from demand', candidate: updatedCandidate });
  }),
  ...
  bulkUpdateStatus: asyncHandler(async (req, res) => {
    ...
    if (!Array.isArray(candidateIds) || !candidateIds.length || !status) {
      return res.status(400).json({ message: 'candidateIds array and status are required' });
    }

    const agencyId = req.user.agencyId;
    const records = await Candidate.find({ _id: { $in: candidateIds }, agencyId })
      .select('_id').lean();
    if (!records.length) return res.status(404).json({ message: 'No candidates found' });

    await Candidate.updateMany({ _id: { $in: records.map(c => c._id) } }, { $set: { status } });
    await Promise.allSettled(records.map(c => computeAndSaveCandidateStatus(c._id)));
    invalidateAlertCache(agencyId);
  ...
  getPrintBundle: asyncHandler(async (req, res) => {
    ...
    const [medical, orientation, insurance, demand, passport] = await Promise.all([
      Medical.findOne({ candidateId: id }).sort({ createdAt: -1 }).lean(),
      Orientation.findOne({ candidateId: id }).sort({ createdAt: -1 }).lean(),
      InsuranceSsf.findOne({ candidateId: id }).sort({ createdAt: -1 }).lean(),
      ...
      candidate.passportId
        ...
        : candidate.passportNumber
          ? Passport.findOne({ passportNumber: candidate.passportNumber, agencyId }).lean()
          : null
    ]);

    res.status(200).json({ candidate, medical, orientation, insurance, demand, passport });
  }),
  ...
  exportBatch: asyncHandler(async (req, res) => {
    ...
    if (ids.length > 2000) {
      return res.status(400).json({ message: 'Maximum 2000 candidates per export' });
    }
    ...
    const [allMedicals, allOrientations, allInsurances] = await Promise.all([
      Medical.find({ candidateId: { $in: candidateIds } }).sort({ createdAt: -1 }).lean(),
      Orientation.find({ candidateId: { $in: candidateIds } }).sort({ createdAt: -1 }).lean(),
      InsuranceSsf.find({ candidateId: { $in: candidateIds } }).sort({ createdAt: -1 }).lean(),
    ]);
