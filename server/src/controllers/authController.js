...
const login = asyncHandler(async (req, res) => {
  ...
  const user = await User.findOne({ email: email.toLowerCase() }).populate('agencyId');
  ...
  if (user.lockedUntil && new Date(user.lockedUntil) > new Date()) {
    ...
    return res.status(403).json({ message: `Account locked. Try again in ${minsLeft} minutes` });
  }
...
const registerAgency = asyncHandler(async (req, res) => {
  const { agencyName, subdomain, adminName, adminEmail, adminPassword } = req.body;

  const existingAgency = await Agency.findOne({ subdomain: subdomain.toLowerCase() });
  if (existingAgency) {
...
const getMe = asyncHandler(async (req, res) => {
  const user = await User.findById(req.user.userId).select('-passwordHash -mfaSecret');
...
const completeInvite = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({
    ...
    agency: agency ? { _id: agency._id, name: agency.name, subdomain: agency.subdomain, plan: agency.plan } : null
  });
});
